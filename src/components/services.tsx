export default function Services() {
    ...
    return (
        <section className='bg-primary py-20 2xl:px-44 px-5'>
            ...
            <div className='grid lg:grid-cols-2 grid-cols-1 lg:mt-20 mt-10 xl:gap-x-20 gap-x-5 gap-y-6'>
                {services.map((service, index) => (
                    <article key={index} className='w-auto bg-gradient-radial from-primary via-primary/90 to-white/10 flex lg:flex-row flex-col xl:h-60 items-center space-x-5 p-4 rounded-2xl border-4 border-grey'>
                        <img className='h-full rounded-xl 2xl:aspect-video lg:aspect-square aspect-video' src={service.image} alt={service.title} />
                        <div>
                            <h4 className='text-white font-bold text-2xl lg:mt-0 mt-3 lg:text-left text-center'>{service.title}</h4>
                            <p className='mt-3 font-normal lg:text-left text-center text-gray-300 text-sm'>{service.description}</p>
                        </div>
                    </article>
                ))}
            </div>
